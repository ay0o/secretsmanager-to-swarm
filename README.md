...
Tool to create Docker secrets whenever a new secret is created in Secrets Manager with the tag `docker: true` or a new value is added to a secret with such tag.

For every key/value pair in the secret, a Docker secret will be created like `<secret_name>_<key>: <value>`.
...
The first one is the simplest and most secure, as the traffic never leaves the AWS network. Creating this VPC Endpoint is not part of this tool either, as it would depend on your infrastructure. If you don't provide a VPC Endpoint, the tool assumes that the standard secrets manager endpoint will be used.
...
## Deploying
Check [variables.tf](variables.tf) for the available required and optional variables.
...
## Known limitations
By design, Docker does not support update operations. That is, you can't create a secret and then modify its value. What does this mean for this tool?
...
So, in summary, it supports creating secrets and adding keys, but it **does not support updating values**.